...
log_file_directory = f'experiment_logs_workstation_journal/workstation_repeat_{repeat_count}/power_logger'
experiment_log_file_directory = f'experiment_logs_workstation_journal/workstation_repeat_{repeat_count}/energy'

num_folds = 1
dataset = ["Hetrec-LastFM",
           "MovieLens-100K",
           "MovieLens-1M",
...
# Läuft nicht durch: "Amazon2018-Electronics", "Amazon2018-Toys-And-Games", "Gowalla",
        ...
        for recommender in recommenders:
            for stage in stages:
                evaluate_log = json.load(
                    open(Path(
                        f"experiment_logs_workstation_journal/workstation_repeat_{repeat_count}/energy/{data_set_name}_{recommender}_0_{num_fold}_{stage}.json"),
                        "r"))

                performance_log = json.load(
                    open(Path(
                        f"experiment_logs_workstation_journal/workstation_repeat_{repeat_count}/data_sets/{data_set_name}/checkpoint_{recommender}/config_0/fold_{num_fold}/evaluate_log.json"),
                        "r"))
...
mapped_csv.to_csv(f"experiment_logs_workstation_journal/workstation_repeat_{repeat_count}/mapped_logs_overall_workstation_journal.csv", index=False)
