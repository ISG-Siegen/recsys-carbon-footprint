...
log_file_directory = 'experiment_logs_leviathan_journal/power_logger'
experiment_log_file_directory = 'experiment_logs_leviathan_journal/energy'
        ...
        for recommender in recommenders:
            for stage in stages:
                evaluate_log = json.load(
                    open(Path(
                        f"experiment_logs_leviathan_journal/energy/{data_set_name}_{recommender}_0_{num_fold}_{stage}.json"),
                        "r"))

                performance_log = json.load(
                    open(Path(
                        f"experiment_logs_leviathan_journal/data_sets/{data_set_name}/checkpoint_{recommender}/config_0/fold_{num_fold}/evaluate_log.json"),
                        "r"))
...
mapped_csv.to_csv(f"experiment_logs_leviathan_journal/mapped_logs_overall_leviathan_journal.csv", index=False)
